/******************************************************************************
 ...
 1) data saving with trigger#, ch#, time in rn####_V1290_EventListAll_evts####.txt
                                        ex) rn20_V1290_EventListAll_evts128.txt
 2) data structure in data file
    buff[Rpnt]=0  TrgCnt=0  Ch0(Tref) = 1769076  Ch1 = 3665  Ch7 = 3658
    buff[Rpnt]=1  TrgCnt=1  Ch0(Tref) = 2034167  Ch1 = 3506  Ch7 = 3503
    buff[Rpnt]=2  TrgCnt=2  Ch0(Tref) = 794719  Ch1 = 3405  Ch7 = 3388
    ....
 ...
 edited by Min Sang RYU 
 << New features >>
 1) correct histogram data saving (Histo_#.txt)
 2) gnuplot - histogram auto-filling
 3) gnuplot - histogram auto-saving
 4) histogram data saving (./data/rn%d_V1290_SavedHisto_ch%d_evts%d.txt")

 2023.02.17 KNU
 edited by Min Sang RYU with Heejeong BYEON, Sehwook LEE
 Original source: V1190Readout-1.1-build20190709.tgz
 << New features >>
 1) V4718 Bridge connection
 2) Time window (hexa 20(500 ns) -28(-700 ns)
 3) Display command menu on the screen
 4) Excuting new gnuplot (not update automatically)
...
//#define OUTALLFILE_NAME   "V1290_EventListAll.txt"
...
int ReadEnd = 0; // if TrgCnt is full, then ReadEnd assigned as 100.
FILE *foutall;
char foutallname[128];
// ###########################################################################
// Functions
// ###########################################################################
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    printf("Board Type0: V%d: SerNum = %d, Fw Revision = %d.%d\n", BoardType, sn, (fwrev >> 8) & 0xFF, fwrev & 0xFF);
    BoardType |= (int)V1190ReadRegister(CR_BOARDID1) << 8;
    printf("Board Type1: V%d: SerNum = %d, Fw Revision = %d.%d\n", BoardType, sn, (fwrev >> 8) & 0xFF, fwrev & 0xFF);
    BoardType |= (int)V1190ReadRegister(CR_BOARDID2) << 16;
    printf("Board Type2: V%d: SerNum = %d, Fw Revision = %d.%d\n", BoardType, sn, (fwrev >> 8) & 0xFF, fwrev & 0xFF);
...
//    V1190WriteOpcode(1, opcd);  // Disable TDC Header/Trailer
    ...
    if (Write2File) {
      ...
      //ryu
      sprintf(foutallname, "./data/rn%d_V1290_EventListAll_evts%d.txt", RunNumber, NumOfTrig);
      printf("FILE OPEN [foutall] = ./data/rn%d_V1290_EventListAll_evts%d.txt \n", RunNumber, NumOfTrig);
      foutall=fopen(foutallname, "w");
    }
	...
	sleep(2);
      ...
      printf(" ========= Calculate throughput rate (every second)   ============= \n");
	...
	if (TrgCnt==0) {
	  printf("No Trigger\n");
	  //	} else {
	} else if (TrgCnt <= NumOfTrig) {  if (TrgCnt == NumOfTrig ) {ReadEnd = 100;}
	  //	}
	  //	if (TrgCnt) {
	  ...
	  for(i=0; i<(NUM_CHANNELS); i++) {
	    ...
	    if (nstat[i] > 0) {
	      ...
	      if (BoardType == 1190) 
		printf(" %d\t%6.2f%%\t%6.2f%%\t\t%6.2f%%\t  nstat=%d m=%.3f (ps)  s=%.3f (ps) --V1190 nstat[i]>0--\n",
		       i, (float)HitCnt[i]*100.0/TrgCnt, (float)DiscardCnt[i]*100.0/TrgCnt,
		       (float)NegCnt[i]*100.0/TrgCnt, nstat[i], 100*mean[i], 100*stddev[i]);
	      else
		printf(" %d\t%6.2f%%\t%6.2f%%\t\t%6.2f%%\t  nstat=%d m=%.3f (ps)  s=%.3f (ps) --V1290 nstat[i]>0--\n",
		       i, (float)HitCnt[i]*100.0/TrgCnt, (float)DiscardCnt[i]*100.0/TrgCnt,
		       (float)NegCnt[i]*100.0/TrgCnt, nstat[i], 25*mean[i], 25*stddev[i]);
	    } else {
		printf("nstat[i]<=0 -- %d\t%6.2f%%\t%6.2f%%\t\t%6.2f%%\t   ---\n",
		       i, (float)HitCnt[i]*100.0/TrgCnt, (float)DiscardCnt[i]*100.0/TrgCnt,
	  ...
	  // added by ryu
	  printf("Number of Trigger (TrgCnt, %lld) has been reached as your requested event number (NumOfTrig, %d). \n\n",
		 TrgCnt, NumOfTrig);
	  goto exit_prog;
	...
	PreviousTime = CurrentTime;
 	...
 	continue;
      ...
      // ryu
      //      printf("totnb = %d, nb = %d \n", totnb, nb);
      printf(" ========= Data Analysis & Saving timerel ============= \n");
      // --------------------------------------------------------------------
      // Data Analysis
      // --------------------------------------------------------------------
      Rpnt = 0;
	...
	if(IS_GLOBAL_HEADER(buff[Rpnt])) {
	  ...
	  if (Write2File) {
	    ...
	    // ryu
	    fprintf(foutall, "buff[Rpnt]=%d  TrgCnt=%lld  ", DATA_EVENT_COUNTER(buff[Rpnt]), TrgCnt);
	    //	    printf("Expected Header (Rpnt: word n. %d), nb/4=%d  \n", Rpnt, nb/4);
	  }
	  Header=0;
	  WordPnt = 1;
	  TrgCnt++;
	  ...
	  if (ChFound[ChTref]) {
	    ...
	    // ryu
	    //	    fprintf(foutall, "Ch%d(Tref) = %d  ", i, timerel);
	    fprintf(foutall, "Ch%d(Tref) = %d  ", ChTref, TimeAbs[ChTref]);
		...
		if (timerel > 0) {
		  ...
		  if (Write2File) {
		    ...
		    fprintf(fout, "Ch %d = %d\n\n", i, timerel);
		    // ryu  
		    fprintf(foutall, "Ch%d = %d  ", i, timerel);
		  }
		  if ((timerel > 0) && ((timerel/HistoBinSize) < HistoNbin))
		    Histo[i][(timerel/HistoBinSize)]++;

		  // ryu -- stop 
		  if ( (TrgCnt == NumOfTrig) && (ReadEnd == 100) )
		    goto exit_prog;
		}
		else {
		  NegCnt[i]++;
		} // end if (timerel > 0)
	      } // end if ((ChFound[i]) && (i != ChTref))
	    } // end for i<NUM_CHANNELS
	    fprintf(foutall, "\n");
	  } // end if (ChFound[ChTref]) 
	  Header=1;
	...
	} // if end -- IS_GLOBAL_HEADER(buff[Rpnt]) -- IS_GLOBAL_TRAILER -- IS_TDC_ERROR -- IS_TDC_DATA -- IS_FILLER
	Rpnt++;
	WordPnt++;
      } // while end (Rpnt<(nb/4))
    } // while end (!Quit) 
